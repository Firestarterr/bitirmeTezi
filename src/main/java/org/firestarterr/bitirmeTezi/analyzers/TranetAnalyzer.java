...
//    transient ProjectService projectService;
    ...
    public void run() throws IOException {
        ...
        core.setName("tranet-core");
        core.setProject(project);
        packages.add(core);

        Package web = new Package();
        web.setName("tranet-web");
        web.setProject(project);
        packages.add(web);

        Package installer = new Package();
        installer.setName("tranet-installer");
        installer.setProject(project);
        packages.add(installer);
    ...
    public Commit createCommitData(String[] commitMessage) throws ParseException {
        ...
        if (commitMessage[2] != null) {
            ...
            if (message.startsWith("#") || Character.isDigit(message.charAt(0))) {
                ...
                String issueName = message.substring(0, firstSpace);
                String cm;
                try {
...
//        developers = developerService.save(developers);
    ...
    public File createFileData(String fileString) {
        ...
        if (fullPath.startsWith("tranet-core")) {
            file.setModule(packages.get(1));
        } else if (fullPath.startsWith("tranet-web")) {
            file.setModule(packages.get(2));
        } else if (fullPath.startsWith("tranet-installer")) {
            file.setModule(packages.get(3));
        } else {
